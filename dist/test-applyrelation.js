...
function applyRelation(nodeTree, relation) {
    ...
    console.log(`\n--- Attempting relation: ${left} = ${right}`);
    console.log(`Original: ${originalString}`);
    // Search for matching subtree and replace
    const findAndReplace = (subtree, depth = 0) => {
        const currentStr = treeToString(subtree);
        console.log(`  [depth ${depth}] Checking: ${currentStr}`);
        ...
        try {
            ...
            console.log(`    ✓ MATCH! Variables: ${JSON.stringify(Object.fromEntries(variableMap))}`);
            console.log(`    Replacement: ${replacedExpr}`);
...
const tests = [
    {
        input: "Sum(k,1,Add(Variable(n),Constant(1)))",
        relation: "Sum(body,from,Add(Variable(n),Constant(1))) = Add(Sum(body,from,n), body\\Add(n,Constant(1)))",
        expected: "Add(Sum(k,1,n),k,Add(n,Constant(1)))"
    },
    {
        input: "Add(Div(a,b),c)",
        relation: "Add(Div(a,b),c) = Div(Add(a,Mult(b,c)),c)",
        expected: "Div(Add(a,Mult(b,c)),c)"
    }
];
console.log("=== Testing applyRelation ===\n");
for (const test of tests) {
    console.log(`\nTest: ${test.input}`);
    console.log(`Relation: ${test.relation}`);
    console.log(`Expected: ${test.expected}`);
    const nodeTree = constructTree(test.input);
    const result = applyRelation(nodeTree, test.relation);
